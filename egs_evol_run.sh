...
set -e
g++ -O3 src/egs_cycle_evol.cpp -o bin/egs_cycle_evol -lgsl -lgslcblas -lm -Wall -Wextra -funsafe-loop-optimizations -xT -fipa-pta -ftracer -march=native -fopenmp
...
timestamp="$(date +"%s")"
cp src/egs_cycle_evol.cpp simdata/egs_cycle_evol_"$timestamp".cpp
...
# Email statistics
#echo "Evolution simulation results for 3-hypercycle simulation" | mutt -a simdata/total_conc.png -s "sim results" -- <email>
